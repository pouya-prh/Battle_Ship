    ...
    ClientBoard(ClientBoard)
{
    ...
    DisplayClientBoard();
    connect(socket,&DialogClient::ArrayRecievedFromServer,this,&DialogClientPlay::DisplayServerBoard);
...
void DialogClientPlay::DisplayServerBoard(int** ServerBoard)
{
    ...
    int Vship32Counter = 0;
    int Vship41Counter = 0;
    this->ServerBoard = socket->returnServerGameBoard();
...
void DialogClientPlay::AttackToServer(int i , int j)
{
    ...
    if (ServerBoard[i][j] == 0 ||ServerBoard[i][j] == 8|| ServerBoard[i][j] == -110 || ServerBoard[i][j] == -210 ||
        ...
        musicPlayer->setSource(QUrl("qrc:/Hit.mp3"));
        musicPlayer->play();
        makeEmptyAround(ServerBoard,-110);
    }
    else if ( ServerBoard[i][j] == 21 ||ServerBoard[i][j] == -21 ){
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        musicPlayer->setSource(QUrl("qrc:/Hit.mp3"));
        ...
        ServerShip21++ ;
        if ( ServerShip21 == 2 ){
            makeEmptyAround(ServerBoard,-210);
        }


    }

    else if ( ServerBoard[i][j] == 22 ||ServerBoard[i][j] == -22){
        ServerBoard[i][j] = -220 ;
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        ...
        ServerShip22++ ;
        if ( ServerShip22 == 2 ){
            makeEmptyAround(ServerBoard,-220);
        }

    }

    else if ( ServerBoard[i][j] == 23||ServerBoard[i][j] == -23 ){
        ServerBoard[i][j] = -230 ;
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        ...
        ServerShip23++ ;
        if ( ServerShip23 == 2 ){
            makeEmptyAround(ServerBoard,-230);
        }

    }

    else if ( ServerBoard[i][j] == 31||ServerBoard[i][j] == -31 ){
        ServerBoard[i][j] = -310 ;
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        ...
        ServerShip31++ ;
        if ( ServerShip31 == 3 ){
            makeEmptyAround(ServerBoard,-310);
        }

    }

    else if ( ServerBoard[i][j] == 32 ||ServerBoard[i][j] == -32){
        ServerBoard[i][j] = -320 ;
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        ...
        ServerShip32++ ;
        if ( ServerShip32 == 3 ){
            makeEmptyAround(ServerBoard,-320);
        }

    }


    else if ( ServerBoard[i][j] == 41 ||ServerBoard[i][j] == -41){
        ServerBoard[i][j] = -410 ;
        QMediaPlayer *musicPlayer = new QMediaPlayer();
        QAudioOutput *output = new QAudioOutput();
        musicPlayer->setAudioOutput(output);
        ...
        ServerShip41++ ;
        if ( ServerShip41 == 4 ){
            makeEmptyAround(ServerBoard,-410);
        }
...
void DialogClientPlay::makeEmptyAround(int** cells,int value)
{    for ( int x = 0 ; x <= 9 ; x++ ){
        for ( int y = 0 ; y<= 9 ; y++ ) {
